...
if(!isset($_GET['parola'])) {
    header("Location: error.php?message=" . urlencode('Parola has to exists'));
    exit();
}

if(empty($_GET['parola'])) {
    header("Location: error.php?message=" . urlencode('Parola cant be empty'));
    exit();
}

if(strlen($_GET['parola']) > 50) {
    header("Location: error.php?message=" . urlencode('Parola cant be long than 50 chars'));
    exit();
}  
...
if(isset($_GET['parola']) && !empty(sanitize_input($_GET['parola']))) {
